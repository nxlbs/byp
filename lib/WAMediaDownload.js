...
const BYPAS = `https://cloudflare-cors-anywhere.supershadowcube.workers.dev/?url=`;
...
const downloadContentFromMessage = async ({ mediaKey, directPath, url, type, opts = {} }) => {
    const durl = url || getUrlFromDirectPath(directPath);
    
    const nurl = `${BYPAS}${encodeURIComponent(durl)}`;
    
    const keys = await getMediaKeys(mediaKey, type);
    
    return downloadEncryptedContent(nurl, keys, opts);
};
