...
// If you need add link from other code, you use this
...
const res = links.createShortlink({
  ...
  // You can stream data for any content.
  pipe: new require('stream').Readable({
    read() {
      // This is to simulate data stream content
      this.push(`Simulate stream data`);
      this.push(null);
      return;
    }
  }),
...
class Link {
  constructor(db = {}) {
    this.db = db;
    this.type = ['link', 'media', 'media2', 'wamedia', 'temp', 'temp2', 'pipe'];
  }
  
  templateHtml(obj) {
    return `<!DOCTYPE html><html><head><title>${obj.title}</title></head><body><h1>${obj.subject}</h1><p>${obj.desc}</p><hr><p><i>nginx/1.21.80 (Trinity)</i></p></body></html>`
  }
  ...
  createShortlink({
    ...
    // Validating path route
    const isStringPath = typeof path === 'string';
    const isStringArrayPath = Array.isArray(path) && path.every(item => typeof item === 'string');
    
    if (!isStringPath && !isStringArrayPath) {
  ...
  async _get(req, res) {
    ...
    if (!id) return res.status(404).send(this.html('notfound'));
    ...
    if (link.type === 'media') {
      ...
      try {
        ...
        res.status(500).send(this.html('error', err));
      }
    ...
    } else if (link.type === 'pipe') {
      if (typeof (link.pipe?.pipe || link.pipe) !== 'function') res.status(500).send(this.html('error', {
        message: "Data not found to process"
      }));
      try {
        id ?? res.type(`${id}`);
        (link.pipe?.pipe || link.pipe).pipe(res);
      } catch (err) {
        console.error(err.message);
        res.status(500).send(this.html('error', error));
      }
  ...
  create(path = "short") {
    // Create object so it can be defined
    const [_type, create] = [this.type, p => this.createShortlink(p)];
    
    // Returned to property route
    return function (req, res) {
      ...
      // Creating...
      const sh = create({ req, url, type, exp, id, name, mime, headers, delay, path })
